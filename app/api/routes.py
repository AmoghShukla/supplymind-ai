...
@router.get("/vendors")
async def vendors(_: User = Depends(current_user), db: AsyncSession = Depends(get_session)): return (await db.scalars(select(Vendor))).all()
@router.post("/vendors")
async def create_vendor(data: VendorCreate, _: User = Depends(require_roles("admin", "analyst")), db: AsyncSession = Depends(get_session)):
    item = Vendor(**data.model_dump()); db.add(item); await db.commit(); await db.refresh(item); return item
@router.get("/shipments")
async def shipments(_: User = Depends(current_user), db: AsyncSession = Depends(get_session)): return (await db.scalars(select(Shipment))).all()
...
async def create_shipment(data: ShipmentCreate, _: User = Depends(require_roles("admin", "analyst")), db: AsyncSession = Depends(get_session)):
    item = Shipment(**data.model_dump()); db.add(item); await db.commit(); await db.refresh(item); return item
...
async def create_incident(data: IncidentCreate, _: User = Depends(require_roles("admin", "analyst")), db: AsyncSession = Depends(get_session)):
    item = Incident(**data.model_dump()); db.add(item); await db.commit(); await db.refresh(item); return item
@router.post("/agents/run")
async def agent_run(data: RunRequest, _: User = Depends(require_roles("admin", "analyst")), db: AsyncSession = Depends(get_session)): return await run_pipeline(db, data.shipment_id, data.auto_execute)
...
async def approvals(_: User = Depends(current_user), db: AsyncSession = Depends(get_session)): return (await db.scalars(select(Approval))).all()
